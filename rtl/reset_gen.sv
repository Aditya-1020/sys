...
module reset_gen #(
	...
	(* keep *) reset_sync_2ff u_sync_io (
		.i_clk(clk),
		.rstn_src(rstn_async),
		.rstn_sync(rstn_io_sync)
	);

	(* keep *) reset_sync_2ff u_sync_dp (
		.i_clk(clk),
		.rstn_src(rstn_async),
	...
	/// stagger release avoid yossys collapse
	always_ff @(posedge clk or negedge rstn_io_sync) begin
		if (!rstn_io_sync) begin
			io_leaf_r <= '0;
		end else begin
			io_leaf_r <= {io_leaf_r[IO_LEAVES-2:0], 1'b1};
		end
	end

	always_ff @(posedge clk or negedge rstn_dp_sync) begin
		if (!rstn_dp_sync) begin
			dp_leaf_r <= '0;
		end else begin
			dp_leaf_r <= {dp_leaf_r[DP_LEAVES-2:0], 1'b1};
		end
	end
