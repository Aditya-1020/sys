# LOCAL COPY OF CTS SCRIPT FROM OPENROAD (EDITED FOR CTS)
...
# LOCAL ADDTION
# librelane cts split the sinks into macro groups and regsier groups liek this:
# [INFO CTS-0011]  Clock net "clk" for macros has 56 sinks.
# [INFO CTS-0011]  Clock net "clk_regs" for registers has 2781 sinks.
# padding the faster group till they arrive together 
# so almost 2800 flops are waiting on 2 slow pins on the sram with 33 delay buffers and 302 dummy loads for~1.2xx transition delay
# two flags insertiondelay and dont use dummy load stops it
append_if_flag arg_list CTS_NO_INSERTION_DELAY -no_insertion_delay
append_if_flag arg_list CTS_DONT_USE_DUMMY_LOAD -dont_use_dummy_load
...
if { [info exists ::env(CTS_DELAY_BUFFER_DERATE_PCT)] } {
    # upstream divides by 100, not 100.0 -- Tcl integer division silently turns
    # any percentage below 100 into 0. Fixed locally so the knob is sweepable.
    lappend arg_list -delay_buffer_derate [expr $::env(CTS_DELAY_BUFFER_DERATE_PCT) / 100.0]
}
