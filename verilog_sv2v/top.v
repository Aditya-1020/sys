...
module top (
	...
	localparam integer LANE_W = 2;
	localparam integer RESULT_W = (2 * DATA_WIDTH) + 2;
	...
	localparam integer ST_FILL_DONE = 3;
	localparam integer ST_CTRL_BUSY = 4;
	localparam integer ST_ARRAY_BUSY = 5;
	localparam integer ST_ARRAY_DONE = 6;
	localparam integer ST_PP_SEL = 7;
	localparam integer ST_W_VALID = 8;
	localparam integer ST_RES_VALID = 9;
	...
	wire arr_ctrl_start;
	wire arr_ctrl_b_en;
	wire arr_ctrl_a_valid;
	wire [LANE_W - 1:0] arr_ctrl_b_lane;
	wire [ROW_W - 1:0] arr_ctrl_b_wdata;
	wire [ROW_W - 1:0] arr_ctrl_ld_a;
	wire arr_ctrl_busy;
	wire arr_ctrl_w_valid;
	wire ping_pong_sel;
	wire array_done;
	wire array_busy;
	...
	wire [5:0] fifo_level;
	wire [31:0] csr_result = {{32 - RESULT_W {fifo_rdata[RESULT_W - 1]}}, fifo_rdata};
	wire wdma_busy;
	wire wdma_done;
	wire wdma_err;
	wire wdma_fifo_rd;
